 ...
 * - Needs to be rewritten when I have more time, a lot of redundant code 8-1-17
 *
 */
...
uint16_t Copy2(FILE *, IMAGEHEAD2 *, uint16_t , uint16_t, FILE *);
int32_t InjectNewStyle(FILE *, IMAGEHEAD *, uint16_t , uint8_t *, uint16_t, FILE *, uint32_t * );
int32_t InjectNewStyle2(FILE *, IMAGEHEAD2 *, uint16_t , uint8_t *, uint16_t, FILE *, uint32_t * );
...
int32_t DecodeLogoBin2(FILE*, IMAGEHEAD2 *);
...
uint32_t IsItTheOnePlus5(FILE *originalLogoBin){
	...
	do{
		fread(&first, 1, SIZEOFLONGINT, originalLogoBin);
		fread(&second, 1, SIZEOFLONGINT, originalLogoBin);

			if(((second - first) % 4096) == 0){
...
IMAGEHEAD2 *ParseHeaders2(FILE *originalLogoBin, uint16_t numberOfOffsets){

	uint8_t i = 0;
	IMAGEHEAD2 *imageHeaders;
...
int32_t DecodeLogoBin2(FILE *originalLogoBin, IMAGEHEAD2 *imageHeaders){
...
int32_t ListFileDetails(FILE *originalLogoBin){
	...
	if(z){
		IMAGEHEAD2 *imageHeaders = ParseHeaders2(originalLogoBin, numberOfOffsets);
 ...
 uint16_t Copy2(FILE *originalLogoBin, IMAGEHEAD2 *imageHeaders, uint16_t numberOfOffsets, uint16_t injectionNumber, FILE *modifiedLogoBin){
...
int32_t InjectNewStyle2(FILE *originalLogoBin, IMAGEHEAD2 *imageHeaders, uint16_t numberOfOffsets, uint8_t *injectionName, uint16_t injectionNumber, FILE *modifiedLogoBin, uint32_t *ihMainOffsets ){
	...
	memset(new.offsets, 0, SIZEOFLONGINT * 32);
...
int32_t InjectNewStyle(FILE *originalLogoBin, IMAGEHEAD *imageHeaders, uint16_t numberOfOffsets, uint8_t *injectionName, uint16_t injectionNumber, FILE *modifiedLogoBin, uint32_t *ihMainOffsets ){
	...
	memset(new.blank, 0, sizeof(new.blank));
	memset(new.metaData, 0, sizeof(new.metaData));
	memset(new.offsets, 0, SIZEOFLONGINT * 28);
	memset(new.name, 0, sizeof(new.name));
	strncpy((char*)new.header, (const char*)HEADER , 8);
	strncpy((char*)new.metaData, (const char*)imageHeaders[injectionNumber].metaData, sizeof(imageHeaders[injectionNumber].metaData));
	strncpy((char*)new.name, (const char*)injectionName, 64);
...
void Usage(){
	...
	fprintf(stdout, "Usage: logoinjector.v.1.52 -i \"input file\" [-l] | [-L] | [-d [-s]] | [-j \"image to be replaced\" [-b] | [-z] | [-s]]\n\n");
	...
	fprintf(stdout, "\t-z This flag forces the use of OP5 headers, it is detected automatically though.\n");
...
int32_t main(int32_t argc, char** argv){
	...
	if (IsItTheOnePlus5(originalLogoBin) == 32){
		
		fprintf(stdout, "\nThis is a OnePlus 5 image.\n\n");
		z = 1;
	}
	...
	if (z){
		...
		if(inject){
			...
			memcpy(&ihMainOffsets , &imageHeaders[0].offsets, SIZEOFLONGINT * 32);
			...
			for (i = 0 ; i < numberOfOffsets ; i++ , injected = 0 ){
				...
				for (j = 0; j < k ; j++){
					...
					if((strcasecmp((const char*)imageHeaders[i].name, (const char*)injectNames[j]) == 0) ||
						...
						if (InjectNewStyle2(originalLogoBin, imageHeaders , numberOfOffsets, imageHeaders[i].name, i, modifiedLogoBin, ihMainOffsets) == 0){
				...
				if (!injected){
					...
					Copy2(originalLogoBin , imageHeaders, numberOfOffsets, i, modifiedLogoBin);
		...
		if (decodeAllOpt){
			...
			DecodeLogoBin2(originalLogoBin, imageHeaders);
			fclose(originalLogoBin);
		...
		if(inject){
			...
			memcpy(&ihMainOffsets , &imageHeaders[0].offsets, SIZEOFLONGINT * 28);
